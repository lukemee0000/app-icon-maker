...
function convertWorkflows(root, output) {
	...
	for (const name of fs
		...
		const description =
			frontmatterDescription(text) || `Run AG Kit workflow ${commandName}`;
		const prompt = `${text}\n\nUser arguments: {{args}}`;
		fs.writeFileSync(
...
export function buildPlugin(root, output) {
	...
	const version = fs
		.readFileSync(path.join(root, ".agents", "VERSION"), "utf8")
		.trim();
	const template = JSON.parse(
		fs.readFileSync(
			path.join(
				...
				"hooks",
				"plugin",
				"gemini-extension.template.json",
			),
			"utf8",
		),
	);
	...
	fs.writeFileSync(
		path.join(output, "gemini-extension.json"),
		`${JSON.stringify(template, null, 2)}\n`,
		"utf8",
	);
	fs.copyFileSync(
		path.join(root, ".agents", "hooks", "plugin", "GEMINI.md"),
		path.join(output, "GEMINI.md"),
	);
	...
	fs.copyFileSync(
		...
		path.join(output, "hooks", "hooks.json"),
	);
	fs.copyFileSync(
		path.join(root, ".agents", "hooks", "validate-tool-call.mjs"),
		path.join(output, "hooks", "validate-tool-call.mjs"),
	);
	fs.copyFileSync(
		path.join(root, ".agents", "mcp_config.json"),
		path.join(output, "mcp_config.example.json"),
	);
	...
	fs.writeFileSync(
		path.join(output, "PLUGIN_CONTENTS.json"),
