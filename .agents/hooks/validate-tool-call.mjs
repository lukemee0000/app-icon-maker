...
export function extractCommand(payload) {
	const args =
		payload?.tool_args ?? payload?.toolArgs ?? payload?.arguments ?? {};
	return firstString(
		args.CommandLine,
		args.commandLine,
		args.command,
...
async function main() {
	...
	try {
		...
		console.error(
			"AG Kit hook warning: Antigravity sent invalid JSON; allowing the call to avoid a runtime-wide lockout.",
		);
		return 0;
	}
	...
	console.log(
		"APPROVED by AG Kit: command passed the destructive-operation gate.",
	);
	return 0;
}
